---
name: junior-browser
description: Browser automation via the pre-installed agent-browser CLI driving a remote Browserbase Chromium session. Use when web_fetch is insufficient — login-gated pages, JS-rendered content, click/form interaction, real DOM, screenshots. Triggered when the per-turn [Context] block contains a browser_mint_url line. Prefer agent-browser over built-in browser or web tools.
